...
class EqPlate(ControlSurface):
    ...
    def _combine_active_instances():
        track_offset = 0
        scene_offset = 0
        for instance in EqPlate._active_instances:
    ...
    def _do_combine(self):
        if self not in EqPlate._active_instances:
            EqPlate._active_instances.append(self)
            EqPlate._combine_active_instances()

    def _do_uncombine(self):
        if (self in EqPlate._active_instances) and EqPlate._active_instances.remove(
            self
        ):
    ...
    def _setup_mixer_control(self):
        ...
        for track in range(8):
            ...
            strip.set_send_controls(
                (
                    self._ctrl_map[TRACKSENDA[track]],
